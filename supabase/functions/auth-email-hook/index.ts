...
const SITE_NAME = 'gwenhwyfar'
const ROOT_DOMAIN = 'bengaudit.nl'
...
const SAMPLE_PROJECT_URL = 'https://gwenhwyfar.lovable.app'
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!resendResponse.ok) {
      console.error('Resend API error', { status: resendResponse.status, data: resendData, run_id })
    ...
    console.log('Auth email sent via Resend', { emailType, email: payload.data.email, run_id, resendId: resendData.id })
    ...
    console.error('Failed to send auth email', { error: errorMsg, run_id })
    await supabase.from('email_send_log').insert({
      message_id: messageId,
      template_name: emailType,
      recipient_email: payload.data.email,
